        ...
        for sensor in online:
            if sensor["computer_name"].lower() == host_to_get.lower():
                ...
                if triage.go_live_session_status:
                    ...
                    # Execute the dir command for each drive letter.
                    for drive_letter in triage.host_drives_list:
                        triage.execute_command(command=r"c:\windows\carbonblack\FGET.exe -extract {0}:\$MFT "
                                                       r"c:\windows\carbonblack\{0}_mft.b"
                                                       .format(drive_letter[0]))
                        # Add the output to an archive.
                        triage.execute_command(command=r"c:\windows\system32\cmd.exe /c c:\windows\carbonblack\7za.exe "
                                                       r"a -tzip mft.zip {0}_mft.b".format(drive_letter[0]))
